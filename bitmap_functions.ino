// a .ino will get processed by the ide, like a .h+.cpp

/***************************************************************************************
** Function name:           drawRAW
** Descriptions:            draw a 565 format 16 bit raw image file
totally copied from https://github.com/Bodmer/TFT_HX8357
***************************************************************************************/

// Tad nasty, assumes globals "eye" for the lcd, SD for the sd card
...
void drawRAW(char *filename, int16_t x, int16_t y, int16_t rawWidth, int16_t rawHeight) {
  ...
  if (!(rawFile = SD.open(filename))) {
    Serial.println(F(" File not found"));
    return;
  }

  // Prepare the TFT screen area to receive the data
  eye.setWindow(x, y, x + rawWidth - 1, y + rawHeight - 1);
  ...
  while(nr--) {
    ...
    eye.pushColors(sdbuffer, RBUFF_SIZE);
  }
  ...
  if (nr) {
    ...
    eye.pushColors(sdbuffer, nr);  // We send nr 16 bit pixels
  }
...
void drawBMP(char *filename, int x, int y, boolean flip) {
  if ((x >= eye.width()) || (y >= eye.height())) return;
  File     bmpFile;
  ...
  if (read16(bmpFile) == 0x4D42) { // BMP file start signature check
    ...
    if ((read16(bmpFile) == 1) && (read16(bmpFile) == 24) && (read32(bmpFile) == 0)) {
        ...
        for (col = 0; col < w; col++) { // For each column...
          ...
          if (sd_ptr >= sizeof(sdbuffer)) {
            ...
            if (tft_ptr) {
              ...
              eye.pushColors(tftbuffer, tft_ptr);
      ...
      if (tft_ptr) eye.pushColors(tftbuffer, tft_ptr);
  ...
  eye.setRotation(rotation); // Put back original rotation
}
